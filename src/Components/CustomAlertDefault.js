...
const CustomAlertDefault = ({ onPressClose, title, showHeader, showHeaderText, message, onPressOkButton, onPressNoButton, showYesButton, showNoButton, yesButtonText, noButtonText, showCloseIcon }) => {
        ...
        <View style={[styles['full_screen'], styles['transparent_black_bg'], styles['centerItems'], styles['absolute_position'], { top: 0, right: 0, left: 0, bottom: 0 }]}>
            <View style={[styles['width_80%'], styles['align_self_center'], styles['bg_white'], styles['padding_10'], { borderRadius: 8 }]}>
                {showHeader && <View style={[styles['width_100%'], { flexDirection: 'row', justifyContent: 'space-between' }]}>
                    {showHeaderText && (<Text style={[styles['width_100%'], styles['font_size_18_semibold'], styles['top_5'], { textAlign: 'center', color: Colors.black },{minHeight:35}]} >{title}</Text>)}
                ...
                <View style={[styles['margin_top_20'], { flexDirection: 'row', flexGrow: 1, justifyContent: 'space-between' }]}>
                    {showYesButton && <CustomButton title={yesButtonText} onPress={onPressOkButton} buttonBg={Colors.purple} btnWidth={showNoButton ? "45%" : '95%'} titleTextColor={Colors.white} />}
                    {(showYesButton && showNoButton) && <CustomButton title={noButtonText} onPress={onPressNoButton} buttonBg={""} btnWidth={showYesButton ? "45%" : '95%'} titleTextColor={Colors.purple} borderWidth={2} borderColor={Colors.purple} borderRadius={5} />}
                    {(showYesButton == false && showNoButton) && <CustomButton title={noButtonText} onPress={onPressNoButton} buttonBg={Colors.purple} btnWidth={showYesButton ? "45%" : '95%'} titleTextColor={Colors.white} />}
                </View>
            </View>
        </View>
    );
...
export default CustomAlertDefault;